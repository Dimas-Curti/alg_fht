...
class Comparator:
    ...
    def compare(self):
        ...
        for i in range(self.height):
            ...
            weightedSum = reduce(lambda m, j: m + (float(self.matrix[i][j]) * float(self.fingerprint[i][j])),
                                 range(self.width), 0)
...
class CompareFHT:
    ...
    def assuranceLevel(self):
        ...
        al = max(sum(cor[0]) / len(cor[0]), sum(cor[1]) / len(cor[1]))
        # al = (sum(cor[0]) / len(cor[0]) + sum(cor[1]) / len(cor[1])) / 2
        return al
